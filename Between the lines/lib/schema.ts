...
export const GuideSchema = z.strictObject({
  ...
  bigQuestion: text,
  claims: z.array(claim).min(10),
  quotes: z.array(z.strictObject({id: text, text, sourceId: text, claimIds: ids, context: text})).min(2).max(5),
...
export function validateResearch(raw: unknown, urls: string[]): Research {
  ...
  for (const s of packet.sources) if (!known.has(canonicalUrl(s.url))) throw new Error('Research source URL missing from provenance');
...
export function validateGuide(raw: unknown, research?: Research, urls?: string[]): Guide {
  ...
  for (const q of g.quotes) {
    ...
    if (!s || q.claimIds.some(id => !claims.get(id)?.sourceIds.includes(q.sourceId))) throw new Error('Quote/source mapping mismatch');
    const key = canonicalUrl(s.url);
